...
public class RobotInputController implements IssObserver {
    ...
    private boolean robotPaused = true;
    private String nextMoveToDo = "moveForward";
    private boolean wasObstacle = false;
...
- a collision between the robot and an obstacle { "collision" : "false", "move": "moveForward"}
    ...
    public synchronized void handleInfo(JSONObject infoJson) {
        ...
        else if (infoJson.has("robotcmd")) handleCommand(infoJson);
    }
    ...
    protected void handleCommand(JSONObject info) {
        String command = (String) info.get("robotcmd");
        if(robotPaused && command.equals("RESUME") && nextMoveToDo != null) {
            robotPaused = false;
            System.out.println("RobotInputController | resuming from pause");
            robotBehaviorLogic.boundaryStep(nextMoveToDo, wasObstacle);
            System.out.println("RobotInputController | resumed from pause");
            nextMoveToDo = null;
        }
        else if(!robotPaused && command.equals("STOP")) {
            robotPaused = true;
        }
    }
